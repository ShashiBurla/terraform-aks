resource "null_resource" "kube-config" {
  ...
  provisioner "local-exec" {
    command = "az aks get-credentials --resource-group devops-practiece --name roboshop-${var.env} --overwrite-existing"
  }
}
    ...
    yamlencode({
      grafana = {
        ingress = {
          ...
          hosts            = ["grafana-${var.env}.shashidevops.online"]
          path             = "/"
          pathType         = "Prefix"
        }
      }
      prometheus = {
        ingress = {
          enabled          = true
          ingressClassName = "traefik"
          hosts            = ["prometheus-${var.env}.shashidevops.online"]
          paths            = ["/"]
          pathType         = "Prefix"
        }
        prometheusSpec = {
